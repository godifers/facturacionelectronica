<?php 
include("lib/phpconexcion.php");
$enlace = conectar_buscadores();
$cad = $_GET['cadena'];
/*$cad = "SELECT LMY_CUENTA,PCU_DESCRIPCION,LMY_FEHA,LMY_SAL_INI,LMY_TOT_DEBE,LMY_TOT_HABER, LMY_SALDO_FIN 
FROM t_libromayor, t_plancuentas where LMY_CUENTA= PCU_CUENTA order by PCU_CUENTA;";*/
set_time_limit(0);
$ejeCadLibMay = mysql_query($cad);

//date_default_timezone_set('American/Ecuador');
require_once"PHPExcel.php";
require_once"PHPExcel/IOFactory.php";

$objPHPExcel = new PHPExcel();
$archivo ="Libro Diario.xls";

$objPHPExcel->getProperties()->setCreator("weblocalhost")
	->setLastModifiedBy("weblocalhost")
	->setTitle("Reporte XLS")
	->setSubject("Reporte")
	->setDescription("")
	->setKeywords("")
	->setCategory("");

$objPHPExcel->getDefaultStyle()->getFont()->setName('Arial Narrow');
$objPHPExcel->getDefaultStyle()->getFont()->setSize(10);
$objPHPExcel->getActiveSheet()->getRowDimension('1')->setRowHeight(20);
$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(10);
$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(30);
$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(60);
$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(20);
$objPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(20);
$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);

$y=1;
$objPHPExcel->setActiveSheetIndex(0)
	->setCellValue("A".$y,'TIPO DOC.')
	->setCellValue("B".$y,'NUM COMPR')
	->setCellValue("C".$y,'CUENTAS')
	->setCellValue("D".$y,'TOTTAL.')
	->setCellValue("E".$y,'TOTAL DEBE.')
	->setCellValue("F".$y,'TOTAL HABER.');

$objPHPExcel->getActiveSheet()
			->getStyle('A1:F1')
			->getFill()
			->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
			->getStartColor()->setARGB('FFEEEEEEE');

	$borders = array(
		'borders' => array(
			'allborders' => array(
				'style' => PHPExcel_Style_Border::BORDER_THIN,
				'color' => array('argb' => 'FF0000000'),
			) 
		), 
	);


while ($row=mysql_fetch_array($ejeCadLibMay)) 
{	
	$y++;
	
	$objPHPExcel->setActiveSheetIndex(0)
	->getStyle('A'.$y.":B".$y.":C".$y.":D".$y.":E".$y.":F".$y)
	->applyFromArray($borders);
	$objPHPExcel->setActiveSheetIndex(0)
	->setCellValueExplicit("A".$y, $row['COM_TIPO_COMPR'],PHPExcel_Cell_DataType::TYPE_STRING)	
	->setCellValueExplicit("B".$y, utf8_encode($row['COM_NUM_COMPROB']),PHPExcel_Cell_DataType::TYPE_STRING)
	->setCellValueExplicit("C".$y, '--',PHPExcel_Cell_DataType::TYPE_STRING)
	->setCellValueExplicit("D".$y,$row['COM_TOT'],PHPExcel_Cell_DataType::TYPE_NUMERIC);

	$cadAsie = "SELECT * from t_asiento, t_plancuentas where PCU_CUENTA = ASI_CUENTA  and ASI_FK_IDCOMPROB = ".$row['IDT_COMPROBANTE'];
	$ejecCadAsiento = mysql_query($cadAsie);
		while ($resA=mysql_fetch_array($ejecCadAsiento)) 
		{	
			$y++;
			
			$objPHPExcel->setActiveSheetIndex(0)
			->getStyle('A'.$y.":B".$y.":C".$y.":D".$y.":E".$y.":F".$y)
			->applyFromArray($borders);
			$objPHPExcel->setActiveSheetIndex(0)
			->setCellValueExplicit("C".$y, utf8_encode($resA['PCU_DESCRIPCION']),PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValueExplicit("E".$y, $resA['ASI_DEBE'],PHPExcel_Cell_DataType::TYPE_NUMERIC)
			->setCellValueExplicit("F".$y, $resA['ASI_HABER'],PHPExcel_Cell_DataType::TYPE_NUMERIC);	
		}
}


header('Content-Type: aplication/vnd.ms-excel');
header('Content-Disposition:attachment;filename="'.$archivo.'"');
header('Cache-Control: max-age=0');
$objWriter= PHPExcel_IOFACTORY::createWriter($objPHPExcel, 'Excel5');
$objWriter->save('php://output');

exit;

 ?>