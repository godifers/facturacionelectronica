...
if ( isset($_SESSION['empresa'])) {
	...
	$num_comp_guias=$_GET['num_comp_guias'];
	$idt_comp_guias=$_GET['idt_comp_guias'];
	$enlace = conectarbd();
	$cad_cant_prod = "SELECT count(*) from t_detalles where DET_ESTADO=1 and DET_FK_IDCOMPROB= ".$idt_comp_guias." and DET_NUM_FACTU ='".$num_comp_guias."'";
	$ejec_cad_cant_prod = mysql_query($cad_cant_prod);
	mysql_close($enlace);
	$res_cant_prod = mysql_fetch_row($ejec_cad_cant_prod);
	$cantidadDeProdcutos = $res_cant_prod[0];
	...
	//---------------------------------------------------------firma de XML-----------------------------------------------------------
	function frimar_xml_guia($num_comp_guias){
		$file=fopen("C:/xml/firmap12/proceso.bat","w") or die("Problemas");
		//vamos añadiendo el contenido
		fputs($file,"start C:/xml/firmap12/firmap12.exe c:/xml/04_GUIAS_DE_REM/".$num_comp_guias.".xml c:/xml/04_GUIAS_DE_REM/FIRMADOS/".$num_comp_guias.".xml c:/xml/luis_gerardo_tipaz_piarpuezan.p12 Carolina2000");
		fclose($file);
		exec('C:\xml\firmap12\proceso.bat');
	}
	//----------------------------------------------------fin de firma de XML de gias de remision--------------------------------------------	
	
	generarXMLguiarem($num_comp_guias, $idt_comp_guias, $cantidadDeProdcutos );
	frimar_xml_guia($num_comp_guias);
	echo '<script>
		window.location="xmlenviados/04_GUIAS_DE_REM/srienvioxml.php?nunf='.$num_comp_guias.'&idcomprobante='.$idt_comp_guias.'&direc=04_GUIAS_DE_REM/FIRMADOS";			
	 </script>';
