...
if ( isset($_SESSION['empresa'])) {	
	...
	$c_idt_usuario = $_POST['c_idt_user'];
	//print_r($_POST);
	...
	$enlace = conectarbd();	
	$call_sp_guardarventa ="CALL SP_GUARDAR_VENTA(".$c_hdidcliente.", ".$subtotal.",".$base0.",".$base12." ,".$iva.",".$totfactura.",
	'". $relacional."', ".$formapago." ,".$c_idt_usuario.", '".$fechaventa."', ".$plazo.", ".$_SESSION['empresa'].", ".$_SESSION['empresa'].",
	 '".$observa."', '".$newfechapag."','".$c_ratiotipo ."',".$c_sumcompra0.",".$c_sumcompra12.",'".$c_num_fact."')";
		...
		//-----------------------------------------------------------------------------------------
			...
			$detalle = $detalles->appendChild($detalle);
				...
				if ($resdetalles['PR_IMPUESTO']==1) {
					...
					$v_unit =  $resdetalles['DET_VAL_UNIT'] / number_format($porcentIVA,2) ;
					$v_tota =  $resdetalles['DET_VAL_TOT'] / number_format($porcentIVA,2) ;
	...
	function  generarXMLguiarem ($num_gia_rem, $numcomprb1, $fechcadena, $fechaventa1 , $tipo_doc , $amb, $id_conduc, $dir_envi, $dir_llega, $fecha_llegada, $ruc_conduc, $nom_condu, $placa_vh, $descrp_vh,$idt_clie_a_eviar,$cant_prod_env){
		...
		$fecha_envio_gia =$newfechapag = date('Y-m-d', strtotime(str_replace('/', '-', $fechaventa1)));
		$enlace =conectarbd();
		$cad_sp_inser_det_gia = "CALL SP_GUARDAR_DET_GIA(".$id_conduc.",'". $dir_envi."', '".$dir_llega."', ".$numcomprb1.",'G', '".$fecha_envio_gia."', '".$fecha_llegada."')";
		...
		$infoGuiaRemision = $guiaRemision->appendChild($infoGuiaRemision);
			...
			$DETG_LUGAR_PARTIDA= $res_detalle_guai[7];// DETG_LUGAR_PARTIDA
			$DETG_LUGAR_DESTINO= $res_detalle_guai[8];// DETG_LUGAR_DESTINO
		...
		$cad_destinatario ="SELECT CP_NOMBRE,CP_APELLIDO,CP_CEDULA,CP_TELEFONO,CP_MAIL, CP_DIRECCION 
	...
	}elseif ($c_ratiotipo=='W' or $c_ratiotipo=='X'){
		echo '<script>
			window.open("pdf_factura_man.php?idt_comp='.$idt_comprob.'&nun_comp='.$res_num_fact_gui.'","_blank");
			window.history.back();
		</script>';
	}
