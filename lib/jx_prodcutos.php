...
<body>
	...
	if ($i==3) {
		...
		where INI_FK_COD_PROD=PR_COD_PROD and INI_VIGENTE=1   and INI_EMPRESA =".$empres."  and  PR_EMPRESA= ".$empres." 
			and (PR_DETALLE like '".str_replace(" ","%",strtoupper($n))."%'
			or PR_PRESENTACION like '%".str_replace(" ","%",strtoupper($n))."%' 
			or concat(PR_DETALLE ,' ', PR_PRESENTACION) like '%".str_replace(" ","%",strtoupper($n))."%'
			or concat(PR_DETALLE ,' ', PR_PRESENTACION) like '%".str_replace(" ","%",strtoupper($n))."%'
			or PR_BARRAS  like '".str_replace(" ","%",strtoupper($n))."%') LIMIT 30";
	} else {
		$cad="SELECT * FROM t_prodcutos ,t_invetario_inicial
		where INI_FK_COD_PROD=PR_COD_PROD and INI_VIGENTE=1 AND PR_ESTADO =1  and INI_EMPRESA =".$empres."  and  PR_EMPRESA= ".$empres." 
			and (PR_DETALLE like '".str_replace(" ","%",strtoupper($n))."%'
			or PR_PRESENTACION like '%".str_replace(" ","%",strtoupper($n))."%' 
			or concat(PR_DETALLE ,' ', PR_PRESENTACION) like '%".str_replace(" ","%",strtoupper($n))."%'
			or concat(PR_PRESENTACION ,' ',PR_DETALLE ) like '%".str_replace(" ","%",strtoupper($n))."%'
			or PR_BARRAS  like '".str_replace(" ","%",strtoupper($n))."%') LIMIT 30";
	}
		...
		//echo $cad;
			...
			while (($resproductod = mysql_fetch_array($ajecqueryprod))) {
				...
				and (COM_FEC_CREA >= '".$fecha_cor_inven."') and DET_FK_IDCOMPROB= IDT_COMPROBANTE and COM_ESTADO_SIS=1 
			   ...
			   <a href="#" onclick="mostrarprodcutos('<?php echo $resproductod['PR_COD_PROD']; ?>','<?php echo $resproductod['PR_DETALLE']; ?>',
			   	...
			   	'<?php echo $resproductod['PR_BARRAS']; ?>','<?php echo $resproductod['PR_VAL_PVP']; ?>','<?php echo $resproductod['PR_FK_PROVEEDOR']; ?>',
			   	'<?php echo $i; ?>','<?php echo $resproductod['INI_STOK_INI']; ?>');" >
